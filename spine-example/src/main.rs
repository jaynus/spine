...
use spine_sys::{
    ...
    enums::{AtlasFilter, AtlasWrap, BlendMode},
    skeleton::{Skeleton, SkeletonData},
    Atlas,
};
use std::{path::PathBuf, time::Instant};
use std140::*;
use ultraviolet as math;
...
pub unsafe fn as_u8_slice<T>(slice: &[T]) -> &[u8] {
...
unsafe fn draw_skeleton(
    ...
    let mut proj = math::projection::orthographic_gl(0.0, 1024.0, 768.0, 0.0, 0.0, 1.0);
    let mvp = proj * math::Mat4::from_scale(0.5);
    let proj_array = mvp.as_array();

    gl.uniform_matrix_4_f32_slice(gl.get_uniform_location(program, "mvp"), false, &proj_array);
            ...
            match attachment.kind() {
                spine::enums::AttachmentType::Region => {
                    ...
                    vertices.push(Vertex {
                        position: vec2(positions[0], positions[1]),
                        uv: vec2(uvs[0], uvs[1]),
                        color,
                    });
                    vertices.push(Vertex {
                        position: vec2(positions[2], positions[3]),
                        uv: vec2(uvs[2], uvs[3]),
                        color,
                    });
                    vertices.push(Vertex {
                        position: vec2(positions[4], positions[5]),
                        uv: vec2(uvs[4], uvs[5]),
                        color,
                    });
                    vertices.push(Vertex {
                    ...
                    vertices.push(Vertex {
                        position: vec2(positions[6], positions[7]),
                        uv: vec2(uvs[6], uvs[7]),
                        color,
                    });
                    vertices.push(Vertex {
...
fn main() {
    unsafe {
        ...
        let (vertex_shader_source, fragment_shader_source) = (
            r#" layout (location = 0) in vec2 a_position;
                ...
                void main() {
                    ...
                    gl_Position = mvp * vec4(a_position, 0.0, 0.0);
                }"#,
            r#"
        ...
        animation.set_by_name("flying", TrackIndex::zero(), true);
